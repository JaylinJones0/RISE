...
// load site.json if present (avoid build failure when missing)
const __filename = fileURLToPath(import.meta.url)
const __dirname = path.dirname(__filename)
let siteConfiguration = {}
try {
  const siteConfigPath = path.join(__dirname, '.figma', 'make', 'site.json')
  if (fs.existsSync(siteConfigPath)) siteConfiguration = JSON.parse(fs.readFileSync(siteConfigPath, 'utf8'))
} catch (e) {
